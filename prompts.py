...
- If the product name/description text is immediately followed, on the same line and within the same cell, by a bare numeric value in parentheses (e.g. "Clopixol 25 mg/100 tabl. (9999999)"), and that number is not a dosage/strength/pack-size (i.e. not paired with mg/ml/tabl./pcs or similar units), exclude the "(number)" from "name". It belongs in candidateProdCode per the rule below, never in "name".
...
- If the product name/description is immediately followed by a bare parenthetical numeric value per the "name" rule above (e.g. "Clopixol 25 mg/100 tabl. (9999999)" -> the "9999999"), include that value in candidateProdCode. This is the ONE exception to the "DO NOT extract codes from parentheses" rule in Product Code Extraction Rules — that rule still applies to productCode (never promote a trailing parenthetical value to productCode, since these are frequently reused/placeholder-like across unrelated products), but candidateProdCode should still capture it as a fallback identifier rather than losing it entirely.
